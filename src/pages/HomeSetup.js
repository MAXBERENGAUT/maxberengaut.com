...
export function setup(){
  ...
  for (let item of items){
    ...
    item.addEventListener("click", e => {
      ...
      setAnimation(zoom, `${styles.zoom} 3.5s forwards`);
      
      // hide items, fade background
      for (let x of items){ setAnimation(x, `${styles.fade_out} 2s forwards`); }
      setAnimation(home, `${styles.white_out} 3s forwards`);
